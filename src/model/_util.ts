...
*/
export function objectDup(data: any): any {
  if (isObject(data)) {
    const obj: any = {}
...
@param path Object keys keys, outermost to innermost.
@param data Object of data (potentially nested).
@return
  `null` if the path can not be traversed to the end;
  else the value at the end of the path.
*/
export function getPath(path: Array<string>, data: any): any {
...
*/
export function setPath(path: Array<string>, value: any, data: any): void {
...
*/
export function calculateDiff(original: any, current: any): any {
  const diff: any = {}
